...
### Changed
- Rebranded public distribution artifacts to WakeVoice: source/wheel packages
  are now `wakevoice-*` and the portable EXE bundle is `WakeVoiceDesktop`.
- Published from the renamed `jsxxwhai/wakevoice` and
  `jsxxwhai/wakevoice-portable` repositories.
